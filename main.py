...
class CalculateValues:
    ...
    def fit(self, order: int = 3):
        ...
        n, d = self.state_df.shape
        std_devs = self.state_df.std()
        bandwidths= (4 / ((d + 2) * n)) ** (1 / (d + 4)) * std_devs
        self.h_x = bandwidths.values
        print(f"'bandwidths': {self.h_x}")
        ...
        self.est_r_sa_actual, self.est_next_psi_SA_actual = self.estimation_r_SA_next_psi_SA(self.h_x, self.state_df, self.action_df, self.next_state_df, self.reward_df, self.p_basis)
    ...
    def calculate_theta_w(self, state_df, action_df, next_state_df, reward_df, w: int = 10, gamma: float = 0.7):
        ...
        r_pi_sa_w_hat.fit(state_df.values, action_df.values.ravel(), reward_df.values.ravel(), w)
        
        # Estimate time-windowed reward  for the actions taken
        est_r_pi_sa_w_actual = r_pi_sa_w_hat(state_df.values, action_df.values.ravel())
    ...
    def estimation_r_SA_next_psi_SA(self, h_x, state_df, action_df, next_state_df, reward_df, basis):
        """
        ...
        r_sa_hat.fit(state_df.values, action_df.values.ravel(), reward_df.values.ravel())
        
        # Estimate expected rewards for the actions taken
        est_r_sa_actual = r_sa_hat(state_df.values, action_df.values.ravel()) 
        
        # Instantiate the next-state basis expectation estimator and fit it to the data
        next_psi_SA = BasisNextSAExpectData(h_x)
        next_psi_SA.fit(state_df.values, action_df.values.ravel(), next_state_df.values, basis)
        
        # Estimate the next-state basis values for the actions taken
        est_next_psi_SA_actual = next_psi_SA(state_df.values, action_df.values.ravel())
    ...
    def estimation_reward(self, h_x, state_df, reward_df):
        ...
        r_pi_hat.fit(state_df.values, reward_df.values.ravel()) # Fit the estimator to the state and reward data
    ...
    def estimation_reward_w(self, h_x, state_df, reward_df, w):
        ...
        r_pi_w_hat.fit(state_df.values, reward_df.values.ravel(), w) # Fit the estimator to the data over the specified window
...
class WilcoxonSignedRankTest:
    def __init__(self, est_Q_actual, est_V, action_df):
        """
        ...
        self.highlight_indices = action_df.values.flatten() == 1
    ...
    def __call__(self, displaying = True, displaying_latex = False):
        ...
        self.stat_lower_1, self.p_value_lower_1 = self.compute_statistics(self.Advantage[self.highlight_indices])
        self.stat_lower_0, self.p_value_lower_0 = self.compute_statistics(self.Advantage[self.highlight_indices == False])
        ...
        # Print statistics and p-values for lower-tailed test
        print(f"\nWilcoxon statistic (lower-tailed test) for mobility restriction period: {self.stat_lower_1}")
        print(f"p-value (lower-tailed test): {self.p_value_lower_1}")
        print(f"Observation periods: {len(self.Advantage[self.highlight_indices])}")
        
        # Print statistics and p-values for lower-tailed test
        print(f"\nWilcoxon statistic (lower-tailed test) for mobility unrestricted period: {self.stat_lower_0}")
        print(f"p-value (lower-tailed test): {self.p_value_lower_0}")
        print(f"Observation periods: {len(self.Advantage[self.highlight_indices == False])}")
        ...
        print("Lower-tailed test (whether the median is less than 0) for mobility restriction period:")
        if self.p_value_lower_1 < alpha:
            print(">> Reject the null hypothesis. The sample median is considered to be less than 0.\n")
        else:
            print(">> Fail to reject the null hypothesis.\n")
            
        print("Lower-tailed test (whether the median is less than 0) for mobility unrestricted period:")
   ...
   # Function to convert a string to a tuple of floats
    ...
    def create_latex_table(self, formatted_results):
        ...
        # Create DataFrame of results
        index_names = ['$W$', '$p$-value', 'stars', '$W$ ($A_{t}=1$)', '$p$-value ($A_{t}=1$)', 'stars ($A_{t}=1$)', '$W$ ($A_{t}=0$)', '$p$-value ($A_{t}=0$)', 'stars ($A_{t}=0$)']
        df = pd.DataFrame(results_array, index=index_names)
        df = df.T
        
        # Convert numerical columns to LaTeX scientific notation
        for col in ['$W$', '$p$-value', '$W$ ($A_{t}=1$)', '$p$-value ($A_{t}=1$)', '$W$ ($A_{t}=0$)', '$p$-value ($A_{t}=0$)']:
            df[col] = df[col].astype(float).apply(self.scientific_to_latex)
        ...
        df['$p$-value ($A_{t}=1$)'] = df.apply(lambda row: self.combine_pval_stars(row, '$p$-value ($A_{t}=1$)', 'stars ($A_{t}=1$)'), axis=1)
        df['$p$-value ($A_{t}=0$)'] = df.apply(lambda row: self.combine_pval_stars(row, '$p$-value ($A_{t}=0$)', 'stars ($A_{t}=0$)'), axis=1)

        # Drop the individual stars columns as they are no longer needed
        df = df.drop(columns=['stars', 'stars ($A_{t}=1$)', 'stars ($A_{t}=0$)'])
